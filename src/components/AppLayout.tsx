...
function UserBlock({ collapsed, onOpen }: { collapsed: boolean; onOpen: () => void }) {
  ...
  return (
    ...
    >
      <div className="font-display grid h-8 w-8 shrink-0 place-items-center rounded-full bg-cyan font-semibold text-primary-deep">
        {initial}
      </div>
      {!collapsed && (
...
/** Collapsible desktop sidebar + a full mobile hamburger drawer from day one
 *  (shadcn `sheet.tsx`, same primitive Pluto's drawer uses) — Chronos/Kairos/
 *  Pluto all shipped this late as a follow-up fix (see ROADMAP.md "mobile-nav
 *  gap"); Hermes builds it correctly from the start instead of repeating that
 *  gap. */
export default function AppLayout() {
  ...
  // definite size). Mirrors the fix Chronos/Kairos/Pluto already carry for
  // their own Aetheris routes — see SUITE-ARCHITECTURE.md §3a.
  ...
  const main = [
    ...
    { to: "/outbox", label: nav.outbox, icon: Inbox },
    { to: "/flows", label: nav.flows, icon: Workflow },
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      >
        ...
        {!collapsed && <div className="signal-rule mx-4" />}
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex items-center justify-between border-b border-sidebar-border bg-sidebar px-4 py-3 md:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="font-display grid h-8 w-8 shrink-0 place-items-center rounded-full bg-cyan text-xs font-semibold text-primary-deep"
            >
              {initial}
            </button>
          </div>
        ...
        <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
          <SheetContent side="left" className="flex w-72 flex-col gap-0 border-sidebar-border bg-sidebar p-0 text-sidebar-foreground">
            ...
            <div className="signal-rule mx-4" />
            <nav className="flex-1 overflow-y-auto px-3 py-4">
              {main.map(({ to, label, icon: Icon }) => (
